...
market(){
  if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
        ...
        echo "Example:"
        echo "	"$0" market langgenius agent 0.0.10"
...
github(){
  if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
        ...
        echo "Example:"
        echo "	"$0" github https://github.com/junjiem/dify-plugin-agent-mcp_sse 0.0.5 agent-mcp_sse.difypkg"
        echo "	"$0" github junjiem/dify-plugin-agent-mcp_sse 0.0.5 agent-mcp_sse.difypkg"
        echo ""
        exit 1
  fi
...
_local(){
  ...
  if [[ -z "$2" ]]; then
        ...
        echo "Example:"
        echo "	"$0" local /root/dify-plugin/langgenius-openai_api_compatible_0.0.8.difypkg"
        echo "	"$0" local ./langgenius-openai_api_compatible_0.0.8.difypkg"
        echo ""
        exit 1
  fi
...
repackage(){
  ...
  PACKAGE_NAME="${PACKAGE_NAME_WITH_EXTENSION%.*}"
	...
	pip3 download -r requirements.txt -d ./wheels --index-url ${PIP_MIRROR_URL}
	sed -i '1i\--no-index --find-links=./wheels/' requirements.txt
	if [ -f .difyignore ]; then
	  sed -i '/^wheels\//d' .difyignore
	fi
	cd ${CURR_DIR}
	chmod 755 ${CURR_DIR}/dify
	${CURR_DIR}/dify plugin package ${CURR_DIR}/${PACKAGE_NAME} -o ${CURR_DIR}/dify-plugin/${PACKAGE_NAME}-offline.difypkg
...
install_unzip(){
  if ! dpkg -s unzip &> /dev/null; then
    echo "Installing unzip ..."
    apt-get update && apt-get install -y --no-install-recommends curl unzip pip && apt-get clean && rm -rf /var/lib/apt/lists/* && rm -rf /tmp/*
    if [ $? -ne 0 ]; then
      echo "Install unzip failed."
      exit 1
    fi
